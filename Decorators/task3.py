import csv
import re
from task2 import logger_decor_path

file_log_path = 'file_3.log'

@logger_decor_path(file_log_path)
def add_dict(contact, contacts_dic):
    #обработка полей ФИО
    if contact[1:3] == ['', '']:
...
def format_phone(contact):
    new_contact = re.sub(r'(\+7|8)?\s*\(?(\d{3})\)?[\s-]?((\d{3}))[\s-]?((\d{2}))[\s-]?(\d{2})(\s*)\(?(доб.)?\s*(\d+)?\)?', r'+7(\2)\4-\6-\7\8\9\10', contact)
    return new_contact
